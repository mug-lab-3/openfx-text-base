...
> [!NOTE]
> 本プロジェクト（テンプレート）では Config 機能はオミットしていますが、
> もととなった [Mug Typography](https://muglab3.booth.pm/items/8339678) では、
> Glaze を使ってユーザー設定を JSON ファイルに保存する **Config 機能** を実現しています。
> Glaze を採用しているのは、その仕組みを本テンプレートにも持ち込めるようにするためでもあります。
