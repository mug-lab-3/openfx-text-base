...
class OpenCLCompositor {
   public:
    ...
    auto composite(void* commandQueuePointer, const void* overlayPixels, void* sourceBuffer, void* destinationBuffer,
                   int width, int height) -> bool;

    // Copy CPU float RGBA pixels into an OpenCL device buffer (CPU fallback path).
    static auto copyHostToDevice(void* commandQueuePointer, const void* hostPixels, void* destinationBuffer, int width,
                                 int height) -> bool;
   ...
   private:
    ...
    auto getOverlayBuffer(cl_command_queue commandQueue, cl_context context, size_t overlayBytes) -> cl_mem;
};
