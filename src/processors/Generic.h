...
class OpenFxToBlend2dProcessor : public OFX::ImageProcessor {
   public:
    OpenFxToBlend2dProcessor(OFX::ImageEffect& effect, BLImage& blendImage,
                             OFX::Image* sourceImage, OfxRectI dstBounds)
        : OFX::ImageProcessor(effect),
          blendImage_(blendImage),
          sourceImage_(sourceImage),
          dstBounds_(dstBounds) {}
...
class Blend2dToOpenFxProcessor : public OFX::ImageProcessor {
   public:
    Blend2dToOpenFxProcessor(OFX::ImageEffect& effect, BLImage& blendImage,
                             OFX::Image* destinationImage)
        : OFX::ImageProcessor(effect),
          blendImage_(blendImage),
          destinationImage_(destinationImage) {
